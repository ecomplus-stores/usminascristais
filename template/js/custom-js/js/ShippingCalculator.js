      ...
      if (!this.zipCode && localStorage) {
        ...
        function getIsoPosition(position) {
            ...
            fetch(url).then(resp => resp.json()).then( (response) => {
                const isoLoc = response.address['ISO3166-2-lvl4']
                if (isoLoc) {
                    const iso3166 = isoLoc.split('-')[0]
                    localStorage.setItem('ISO', iso3166)
                    this.localIso = iso3166
                }
